...
local function invoke_replacement(prompt_bufnr)
  return function()
    ...
    if change.type == constants.change_type.QUICK_REPLACE then
      plugin.quick_replace(change.method_desc)
...
local function telescope_textcase(opts)
  ...
  for _, method in pairs({
    ...
    local quick_replace_table = {
      display = 'Convert to ' .. method.desc,
      method_desc = method.desc,
      type = constants.change_type.QUICK_REPLACE,
    }
    ...
    table.insert(results, quick_replace_table)
